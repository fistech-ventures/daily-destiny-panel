...
export const Toolbox = {
  ...
  generateSlug: function (selector: string, separator: string = '-', isRandom: boolean = true): string {
    ...
    if (type === 'bn') {
      sanitizedSelector = sanitizedSelector.normalize('NFD').replace(/[\u0300-\u036f]/g, '');
    ...
    return `${sanitizedSelector}${isRandom ? separator + this.generateKey(8, 'key') : ''}`;
  },
