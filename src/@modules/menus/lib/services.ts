...
import { IMenu, IMenuCreate, IMenusFilter, IMenusResponse } from './interfaces';

const END_POINT: string = '/menu';

export const MenusServices = {
  NAME: END_POINT,

  findById: async (id: TId): Promise<IBaseResponse<IMenu>> => {
    try {
  ...
  find: async (options: IMenusFilter): Promise<IMenusResponse> => {
    try {
  ...
  create: async (payload: IMenuCreate): Promise<IBaseResponse<IMenu>> => {
    try {
  ...
  update: async (payload: { id: TId; data: Partial<IMenuCreate> }): Promise<IBaseResponse<IMenu>> => {
    try {
