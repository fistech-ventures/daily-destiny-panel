...
import { PagesHooks } from '../lib/hooks';
...
const PagesList: React.FC<IProps> = ({ isLoading, data, pagination }) => {
  const [messageApi, messageHolder] = message.useMessage();
  const [formInstance] = Form.useForm();
  const [updateItem, setUpdateItem] = useState<IPage>(null);
  ...
  const pageUpdateFn = PagesHooks.useUpdate({
    config: {
      onSuccess: (res) => {
        if (!res.success) {
  ...
  const pageDeleteFn = PagesHooks.useDelete({
    config: {
      onSuccess: (res) => {
        if (!res.success) {
          messageApi.error(res.message);
        ...
        return (
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
            ...
            <Button
              danger
              onClick={() => {
                getAccess(['cms-pages:delete'], () => {
                  setConfirmationDialog({
                    open: true,
                    title: 'Delete Page',
                    content: `Are you sure you want to delete "${item.title}"?`,
                    onConfirm: () => {
                      pageDeleteFn.mutate(item.id);
                      setConfirmationDialog({ open: false, title: '', content: '', onConfirm: () => {} });
                    },
                  });
                });
    ...
    <React.Fragment>
      ...
      >
        <PagesForm
          ...
          isLoading={pageUpdateFn.isPending}
          onFinish={(values) =>
            pageUpdateFn.mutate({
              id: updateItem?.id,
              data: values,
