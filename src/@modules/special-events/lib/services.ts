...
import {
  ISpecialEvent,
  ISpecialEventCreate,
  ISpecialEventsFilter,
  ISpecialEventsResponse,
} from './interfaces';

const END_POINT: string = '/special-events';

export const SpecialEventsServices = {
  NAME: END_POINT,

  findById: async (id: TId): Promise<IBaseResponse<ISpecialEvent>> => {
    try {
  ...
  find: async (options: ISpecialEventsFilter): Promise<ISpecialEventsResponse> => {
    try {
  ...
  create: async (payload: ISpecialEventCreate): Promise<IBaseResponse<ISpecialEvent>> => {
    try {
  ...
  update: async (payload: { id: TId; data: Partial<ISpecialEventCreate> }): Promise<IBaseResponse<ISpecialEvent>> => {
    try {
