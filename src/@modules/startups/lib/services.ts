...
import { IStartup, IStartupCreate, IStartupsFilter, IStartupsResponse } from './interfaces';

const END_POINT: string = '/startups';

export const StartupsServices = {
  NAME: END_POINT,

  findById: async (id: TId): Promise<IBaseResponse<IStartup>> => {
    try {
  ...
  find: async (options: IStartupsFilter): Promise<IStartupsResponse> => {
    try {
  ...
  create: async (payload: IStartupCreate): Promise<IBaseResponse<IStartup>> => {
    try {
  ...
  update: async (payload: { id: TId; data: Partial<IStartupCreate> }): Promise<IBaseResponse<IStartup>> => {
    try {
