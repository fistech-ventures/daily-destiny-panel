...
import { ISubCategoriesFilter, ISubCategoriesResponse, ISubCategory, ISubCategoryCreate } from './interfaces';

const END_POINT: string = '/sub-categories';

export const SubCategoriesServices = {
  NAME: END_POINT,

  findById: async (id: TId): Promise<IBaseResponse<ISubCategory>> => {
    try {
  ...
  find: async (options: ISubCategoriesFilter): Promise<ISubCategoriesResponse> => {
    try {
  ...
  create: async (payload: ISubCategoryCreate): Promise<IBaseResponse<ISubCategory>> => {
    try {
  ...
  update: async (payload: { id: TId; data: Partial<ISubCategoryCreate> }): Promise<IBaseResponse<ISubCategory>> => {
    try {
